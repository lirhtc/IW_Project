...
consoleCenter.prototype.converSkillToBuff = function (skill, party, enemy, target, castActor) {
	...
	var buffsArray = seedsArray.map(function (seed) {
			return this.convertSeedToBuff(seed, party, enemy, target, castActor)
		}.bind(this));
	console.log (buffsArray);
	buffsArray.forEach (function (e){
		var target = this.findTarget(e, party, enemy, target, castActor);
		this.pushBuffsToTargets(e, target)
	}.bind(this));
};
...
consoleCenter.prototype.getSkillSeeds = function (generalItem) {
	...
	else if (generalItem && generalItem.basic.hasOwnProperty('skillSeeds')) {
...
consoleCenter.prototype.convertSeedToBuff = function (seed, party, enemy, target, castActor) {
	...
	tempBuff.buffParameters[seed.targetParameter] = seed.effects;
	return tempBuff;
};

consoleCenter.prototype.pushBuffsToTargets = function (buffsArray, target) {
	if (Array.isArray(target)) {
		target.forEach(function (e, i, A) {
			if (buffsArray.buffType === "parameterChange") {
				var parameterKey = buffsArray.targetParameter;
				buffsArray.buffParameters[parameterKey] = buffsArray.effects;
				if (!target.buff) {
					target.buff = {};
				}
				...
				target.buff.parameterChange.push(buffsArray);
			}
		}.bind(this));
	} else {
		if (buffsArray.buffType === "parameterChange") {
...
consoleCenter.prototype.findTarget = function (seed, party, enemy, target, castActor) {
	...
	if (party){var targetGroup = party.containsMember(target) ? party : enemy;}
	if (enemy){var casterGroup = party.containsMember(castActor) ? party : enemy;}
